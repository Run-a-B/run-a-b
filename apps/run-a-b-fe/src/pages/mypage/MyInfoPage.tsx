...
export default function MyInfoPage() {
    ...
    const pwError =
        pw.next.length > 0 && !PASSWORD_REGEX.test(pw.next)
    ...
    return (
        <div className="max-w-xl">
            {/* 저장 완료 토스트 */}
            <div
                className={`fixed bottom-8 right-8 z-50 flex items-center gap-3 bg-gray-900 text-white text-sm font-medium px-5 py-3.5 rounded-2xl shadow-xl transition-all duration-300 ${saved ? "opacity-100 translate-y-0" : "opacity-0 translate-y-3 pointer-events-none"}`}
            >
            ...
            <section className="border border-red-200 bg-red-50 rounded-xl px-5 py-4">
                ...
                <p className="text-xs text-gray-500 mb-3">
                    계정을 삭제하면 모든 리포트, 저장된 정책 정보가 영구적으로
